...
" dein with toml support natively,
" but yaml is more compact.
...
" add a command to edit plugin settings quickly {{{

let s:plugin_setting_dirname = expand('$VIMPATH/core/plugins/')

function! s:edit_plugin_setting(plugin_name)
    if empty(a:plugin_name)
        execute 'edit '  . $VIMPATH . '/core/plugins.yaml'
    else
